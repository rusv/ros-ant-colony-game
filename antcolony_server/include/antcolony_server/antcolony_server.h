...
public:
    ...
    void setInitialState(antcolony_msgs::State &state, int player_one_id, std::string token_one,
                         int player_two_id, std::string token_two);
...
private:
    ...
    void generateNeighbors(int x, int y, std::vector<std::pair<int, int>> &neighbors) const;

    // Find next move on the way from start to end
    bool searchPath(int start_x, int start_y, int end_x, int end_y, int &next_x, int &next_y) const;
    ...
    void nearestUnoccupied(int start_x, int start_y, int &free_x, int &free_y) const;
...
private:
    ...
    bool register_call(antcolony_msgs::RegisterRequest &request, antcolony_msgs::RegisterResponse &response);

    // Callback to submit an order for the current game tick with exclusive access
    bool play_call(antcolony_msgs::PlayRequest &request, antcolony_msgs::PlayResponse &response);
